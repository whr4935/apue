    ...
    if (IsHTTPLiveURL(url)) {
        ...
        mDataSourceType = DATA_SOURCE_TYPE_RTSP;
                ...
                || !strncasecmp(url, "https://", 8))
                    ...
                    || strstr(url, ".sdp?"))) {
        source = new RTSPSource(
                notify, url, headers, mUIDValid, mUID, true);
        ...
        status_t err = genericSource->setDataSource(url, headers);
...
void NuPlayer::notifyListener(int msg, int ext1, int ext2, const void *in) {
    if (mDriver == NULL) {
        return;
    }
