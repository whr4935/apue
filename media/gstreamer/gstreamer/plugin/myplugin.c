...
static GstStaticPadTemplate s_src_factory = GST_STATIC_PAD_TEMPLATE("src_%u",
    GST_PAD_SRC,
    GST_PAD_SOMETIMES,
    GST_STATIC_CAPS("ANY")
    );
...
gst_plugin_template_class_init (GstPluginTemplateClass * klass)
{
  ...
  gst_element_class_add_pad_template(&gstelement_class,
        gst_static_pad_template_get(&s_src_factory));
}
...
{
  GstPluginTemplate *filter;
  static gsize size_count;

  size_count += gst_buffer_get_size(buf);

  filter = GST_PLUGIN_TEMPLATE (parent);

  if (filter->silent == FALSE)
    //g_print ("I'm plugged, therefore I'm in.\n");
      g_print("Have data of size %" G_GSIZE_FORMAT " bytes.\r", size_count);
//  fflush(stdout);
...
plugin_init (GstPlugin * plugin)
{
  ...
  return gst_element_register (plugin, "test", GST_RANK_NONE,
