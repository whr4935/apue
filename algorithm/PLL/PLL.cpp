...
static const size_t kMinSamplesToStopPrime = 8;
static const size_t kMinSamplesToEstimatePeriod = 3;
static const size_t kMaxSamplesToEstimatePeriod = 8;
...
void PLL::test() {
    ...
    // b = -0.14097(2)+
    // err = 0.2750578(703)+
    ALOGI("a=%lld (%.6f), b=%lld (%.6f), err=%lld (%.6f)",
...
void PLL::prime(size_t numSamplesToUse) {
    if (numSamplesToUse > mNumSamples) {
        numSamplesToUse = mNumSamples;
    }
    ...
    for (size_t i = 0; i < numSamplesToUse; ++i) {
        ...
        if (i > 0) {
            if (time - lastTime > kMinPeriod) {
                ...
                deltas.push_back(time - lastTime);
            }
        } else {
            firstTime = time;
        }
    ...
    std::sort(deltas.begin(), deltas.end(), compare<nsecs_t>);
    size_t numDeltas = deltas.size();
    if (numDeltas > 1) {
        nsecs_t deltaMinLimit = std::max(deltas[0] / kMultiplesThresholdDiv, kMinPeriod);
...
nsecs_t PLL::addSample(nsecs_t time) {
    if (mLastTime >= 0
